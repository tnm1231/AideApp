...
class TaskStatusView(MethodView):
        ...
        if result:
            ...
            return jsonify({
                ...
                "data": {
                    ...
                    "removed_files": result.removed_files.split('\n') if result.removed_files else [],
                    "changed_files": result.changed_files.split('\n') if result.changed_files else [],
                    "algorithm": result.algorithm.split('\n') if result.algorithm else [],
                    "start_time" : result.start_time or "N/A",
                    "end_time" : result.end_time or "N/A",
                    "run_time" : result.run_time or "N/A"
                }
            }), 200
        else:
            return jsonify({
    ...
    def save_result(task_id, scan_result):
        ...
        try:
            ...
            start_time = output.get("start_time",None)
            run_time = output.get("run_time_seconds", None)

            added_files = output.get("added", {}) or {}
            removed_files = output.get("removed", {}) or {}
            changed_files = output.get("changed", {}) or {}
            details = output.get("details", {}) or {}
            databases = output.get("databases", {})
            database_path = next(iter(databases), None)
            hash_data = databases.get(database_path, {})
            
            algorithm_data = json.dumps(hash_data)  # Ensure JSON serialization
            details_json = json.dumps(details)  # Convert to JSON
            added_files_json = json.dumps(added_files)  
            removed_files_json = json.dumps(removed_files)  
            ...
            result = ResultScan(
                ...
                added_files=added_files_json,
                removed_files=removed_files_json,
