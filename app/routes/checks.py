...
class CheckView(MethodView):
    ...
    def post(self):
        ...
        custom_config_path = os.path.normpath(os.path.join(base_path, custom_config))

        if not custom_config_path.startswith(base_path):
        ...
        elif check_type == "customConfig" and custom_config_path:
            command.extend(["--config", custom_config_path])
        elif check_type == "customConfigAndFile" and custom_config_path and specific_file:
            command.extend(["--config", custom_config_path, "--limit", specific_file])
